...
$sql2 = "SELECT count(`id`) AS `sum` FROM `view_borrowreturn_hdr` WHERE STATUS = 'รออนุมัติ'";
$result2 = $conn->query($sql2);
$row2 = $result2 ->fetch_assoc();

if($_SESSION['role'] == 'ผู้อนุมัติ' || $_SESSION['role'] == 'ผู้ดูแล') { 
    $sql = "SELECT * FROM `view_borrowreturn_hdr` WHERE `STATUS` = 'รออนุมัติ' ORDER BY `id` ASC";
    $result = $conn->query($sql);
} else {
  $sql = "SELECT * FROM `view_borrowreturn_hdr` WHERE `STATUS` = 'รออนุมัติ' AND `user` =  '".$_SESSION['id']."' ORDER BY `id` ASC";
  $result = $conn->query($sql);
}
  ?>
    ...
    <section class="content">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title d-inline-block">รายการอนุมัติการขอเบิก</h3>
         <a href="" class="btn btn-info float-right btn-lg disabled">รออนุมัติ <span class="badge badge-warning"> <?php echo $row2['sum']; ?> เอกสาร </span> </a>
        ...
        <div class="card-body table-responsive">
          <table id="dataTable" class="table table-bordered thead-light">
            ...
            <tr>
              ...
              <th class="text-center">ขอเบิกโดย</th>
